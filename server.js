...
const ORGANIZER_CONTACT = '+996 555 000 000';
...
db.serialize(() => {
  ...
  // Гарантируем serial_no + уникальный индекс (только для NOT NULL)
...
// Ждём появления файла (чтобы не открыть до записи)
...
async function renderPdf(ticket) {
  const data = {
    ...
    year: dayjs().year(),
    // логотип из public/img/logo.png
    logo_data_url: imageAsDataUrl('img/logo.png'),
    // QR
    qr_data_url: await makeQrDataUrl(
      JSON.stringify({ ticket_id: ticket.id, event: EVENT_NAME })
    ),
  };
...
app.post('/api/tickets', requireAuth, (req, res) => {
  ...
  if (!buyer_name || buyer_name.trim().length < 2) {
    return res
      .status(400)
      .json({ error: 'Укажи имя покупателя (минимум 2 символа).' });
  }
  ...
  db.run('BEGIN IMMEDIATE TRANSACTION', beginErr => {
      ...
      db.run(
        ...
        function (updErr) {
          ...
          if (this.changes === 0) {
            ...
            return res
              .status(409)
              .json({ error: 'Номер недоступен, выбери другой или оставь пустым' });
          }
          ...
          db.run(
            ...
            async insErr => {
              if (insErr) {
                db.run('ROLLBACK');
                return res.status(500).json({ error: 'Insert ticket failed' });
              }
    ...
    if (requestedNo) {
      if (requestedNo < 1 || requestedNo > MAX_TICKETS) {
        ...
        return res
          .status(400)
          .json({ error: `Номер должен быть от 1 до ${MAX_TICKETS}` });
      }
      ...
      db.get(
        ...
        (selErr, row) => {
          ...
          if (!row) {
            db.run('ROLLBACK');
            return res.status(409).json({ error: 'Свободных номеров не осталось' });
          }
...
app.get('/export.csv', requireAuth, (_req, res) => {
  db.all(
    ...
    (err, rows) => {
      ...
      const header =
        'serial_no,id,buyer_name,ticket_category,status,issued_at,used_at';
...
app.get('/download/:id', async (req, res) => {
  ...
  try {
    ...
    return res
      .status(404)
      .send('PDF ещё готовится, попробуйте скачать через секунду.');
  }
});
...
app.get('/view/:id', async (req, res) => {
  ...
  try {
    ...
    return res
      .status(404)
      .send('PDF ещё готовится, попробуйте открыть через секунду.');
  }
});

// Проверка QR + отметка used
  ...
  db.get(
    ...
    (err, row) => {
      ...
      db.run(
        ...
        function (uerr) {
          if (uerr)
            return res.status(500).json({ status: 'ERROR', error: uerr.message });
          if (this.changes === 0) {
            return res.status(200).json({
              status: 'ALREADY_USED',
              ticket_id: row.id,
...
app.get('/scan', requireAuth, (_req, res) =>
